...
# ====== Chat state (per-phase 15-turn limit) ======
...
def metrics_for_goal(goal_id, start, end):
    ...
    attempts = [a for a in ATTEMPTS if a["goal_id"] == goal_id and within_range(a["timestamp"], start, end)]
    avg3 = None
    if attempts:
        scores = [a["score"] for a in attempts][-3:]
        avg3 = round(sum(scores) / len(scores), 1)
    # Most common error
    err_df = error_frequency(ATTEMPTS, start, end, goal_filter=goal_id)
    common_err = None
    if not err_df.empty:
        err_row = err_df.sort_values("count", ascending=False).iloc[0]
        common_err = f"{err_row['error']} ({int(err_row['count'])}x)"
    # Time on goal
    time_sum = sum(a["time_spent_mins"] for a in ATTEMPTS if a["goal_id"] == goal_id and within_range(a["timestamp"], start, end))
...
def coach_reply(phase, goal_id, start, end):
    ...
    if phase == "forethought":
        tips = [
            ...
            f"Most common mistake: {m['common_err'] or 'none in this range'}.",
        ...
        tips = [
            ...
            f"Top mistake to fix next: {m['common_err'] or 'none'} — focus on this first.",
            ...
            "If you miss a question, write the right answer and a one-line reason."
        ]
    else:  # reflection
        tips = [
            f"From your notes and scores for {goal_id}, your recent average (last 3) is {m['avg3']} (if any).",
            f"Mistake trend: {m['common_err'] or 'no repeating mistake spotted'}.",
...
def seed_once():
    if not st.session_state.seed_done:
        ...
        st.session_state.chat["performance"] = [
            ...
            ("agent", "Pick one mistake to fix. What will you do differently in the next 10 minutes?"),
        ]
...
sel_goal_effective = sel_goal if sel_goal != "ALL" else "Goal 1"
...
with tab_fore:
    ...
    g = next(
        (x for x in GOALS if x["goal_id"] == sel_goal_effective),
        GOALS[0],
    )
    window_days = max(
        1,
    ...
    chat_tab("forethought", sel_goal_effective, start_str, end_str)
...
with tab_perf:
    ...
    chat_tab("performance", sel_goal_effective, start_str, end_str)
...
with tab_refl:
    ...
    chat_tab("reflection", sel_goal_effective, start_str, end_str)
