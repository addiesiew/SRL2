...
def seed_once():
    if not st.session_state.seed_done:
        ...
        # Overall coach: 15 seeded lines + limit line
        st.session_state.chat["overall"] = [
            ...
            ("user", "Where should I focus first?"),
            ("agent", "Errors with labels appear most. Start by fixing one label type at a time."),
            ("user", "How long should I spend?"),
            ("agent", "About 10–15 minutes on a short task, then a quick quiz."),
            ("user", "What small goal can I set for tomorrow?"),
            ("agent", "For example: explain the job of three cell parts in your own words."),
            ("user", "How can I tell I am improving?"),
            ("agent", "Watch if your quick quiz score nudges up and the error bar gets shorter."),
            ("user", "What about my reflections?"),
            ("agent", "Keep writing one thing that worked and one to change. Your feeling trend is going up."),
            ("user", "Should I switch goals or finish one first?"),
            ("agent", "Finish a small step in your current goal. Then switch if it helps your top errors."),
            ("user", "Okay, I will set a time for tomorrow."),
        ]
...
# Overview coach just below radar
