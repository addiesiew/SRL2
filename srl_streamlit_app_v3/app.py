...
def chat_tab(phase):
    ...
    st.subheader(f"{phase.capitalize()} Coach (max 3 turns)")
    if status["blocked"]:
...
colA, colB, colC, colD = st.columns([1,1,1,0.6])
with colA:
...
# Small overview radar above tabs (helps orient); phases live inside tabs
...
# ============================ Three Phase Tabs ============================
tab_fore, tab_perf, tab_refl = st.tabs(["Phase 1 • Forethought", "Phase 2 • Performance", "Phase 3 • Reflection"])

with tab_fore:
    st.header("Forethought (Goal Setting & Planning)")
    ...
    bars_df = pd.DataFrame(rows).melt(id_vars=["task"], value_vars=["EstDays","ActDays"], var_name="Type", value_name="Days")
...
with tab_perf:
    st.header("Performance (Monitoring & Feedback)")
...
with tab_refl:
    st.header("Reflection (Strategies & Sentiment)")
    c1, c2 = st.columns([1,1])
    with c1:
        st.subheader("Strategy Word Cloud")
        img = wordcloud_image(REFLECTIONS, goal_filter=None if sel_goal=="ALL" else sel_goal)
        st.image(img, caption="Two-color legend (conceptual): green=effective, red=less effective")
    with c2:
